...
/**
 * 生成 ts 文件，用于代码补全
 * @param directory service 文件扫描路径
 * @param tsFile ts 文件保存路径
 * @param patterns 匹配规则
 */
export async function findServicesToTyping(directory: string, tsFile: string, patterns?: string) {
  ...
  for (const fullpath of await globby(patterns || '**/*.{ts,js}', { cwd: directory, absolute: true })) {
    ...
    code.push(`import ${className} from '${importFilename}';`);
